...
void dfsOrdTopol(Graph &g, vi &visited, int v, vi &vetOrdTop, int  *indice){
	...
	for (int i = 0; i < g.adjList[v].size(); ++i)
	{
		...
		if (visited[w.first] == UNVISITED)
			dfsOrdTopol(g,visited,w.first, vetOrdTop, indice);
	}
	vetOrdTop[*indice] = v;
	(*indice)--;
}
...
void ordtopologica(Graph &g, vi &visited, Vertex v){
	// neste vetor colocaremos os vertices ordenados...
	vi vetOrdenacao(g.nVert);
	int indice = g.nVert-1;  // o vetor será preenchido do final para o comeco

	for (int i = 0; i < visited.size(); ++i)
		if (visited[i] == UNVISITED){
			dfsOrdTopol(g,visited, i, vetOrdenacao, &indice);
		}

	// imprime a ordenacao topologica
	for (int i = 0; i < vetOrdenacao.size(); ++i){
		printf("%d ", vetOrdenacao[i]);
	}
	printf("\n");
}
